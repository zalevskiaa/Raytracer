...
bool AllEqual(const std::vector<std::string>& a, const std::vector<std::string>& b) {
    if (a.size() != b.size()) {
        return false;
    }
...
TEST_CASE("Tokenize", "[raytracer]") {
    std::vector<std::string> result = TokenizeSlash("1/2/3");
    REQUIRE(AllEqual(result, {"1", "2", "3"}));

    result = TokenizeSlash("1//3");
    REQUIRE(AllEqual(result, {"1", "", "3"}));

    result = TokenizeSlash("1/2/");
    REQUIRE(AllEqual(result, {"1", "2", ""}));

    result = TokenizeSlash("/2/3");
    REQUIRE(AllEqual(result, {"", "2", "3"}));

    result = TokenizeSlash("//");
    REQUIRE(AllEqual(result, {"", "", ""}));

    result = TokenizeSlash("1/2");
    REQUIRE(AllEqual(result, {"1", "2"}));

    result = TokenizeSlash("1/");
    REQUIRE(AllEqual(result, {"1", ""}));

    result = TokenizeSlash("/2");
    REQUIRE(AllEqual(result, {"", "2"}));
